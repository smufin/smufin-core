...
typedef std::function<void(rocksdb::DB* db, sm_idx_set set, int pid, int fid)> load_f;
...
private:
    moodycamel::ConcurrentQueue<std::pair<int, int>> _filter_queue;

    void load(sm_idx_type type, sm_idx_set set);
    void load_filters(load_f load_filter, rocksdb::DB* db, sm_idx_set set);

    void load_seq(rocksdb::DB* db, sm_idx_set set, int pid, int fid);
    void load_k2i(rocksdb::DB* db, sm_idx_set set, int pid, int fid);
    void load_i2p(rocksdb::DB* db, sm_idx_set set, int pid, int fid);
};
